<?
	...
	if(isset($_POST['cat']) ||  isset($_POST['quest'])  ){
		...
		if(!empty($quest)  ){
			$sql = mysql_query("INSERT INTO `foxycare`.`answer` (`id`, `uid`, `ques_id`, `ans`, `datetime`) VALUES (NULL, '".$_SESSION['id']."', '".$category."', '".$quest."', '".$date."')");
			$rid = mysql_insert_id();
			
			
			
				$sql1 = mysql_query("SELECT * FROM `answer` WHERE `id`='".$rid."' ");
		...
		$rows = mysql_fetch_assoc($sql2);
			...
			echo '<div class="entrys" id="'.$id.'">
						...
						<div class="cnt">
						<h4 style="font-size:16px; font-family:Roboto,trebuchet ms,serif !important; padding-top:10px;">'.$rows['name'].'</h4>
