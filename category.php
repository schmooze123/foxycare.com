...
?>
  <?
$sql5 = mysql_query("SELECT * FROM `category` WHERE `id` = '".$_GET['cat']."' ");
$fetch15 = mysql_fetch_assoc($sql5);
...
$.post("category.php?action=get&last_msg_id="+ID+"&cat="+<? echo $_GET['cat']?>,
function( data )
{
if (data != "") 
{
...
jQuery(
  function($)
  {
    $('.wrapper').bind('scroll', function()
					...
					<h3><? echo $fetch15['name'] ?></h3>
                     
                    <?
$sql = mysql_query("SELECT * FROM `posts` WHERE `category`='".$_GET['cat']."' ORDER BY `id` DESC LIMIT 10");
