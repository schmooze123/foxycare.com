...
<li><a href="full_post.php" style="padding:5px;">More Posts</a></li>
			...
			$sql = mysql_query("SELECT * FROM `question` WHERE `id`='".$_GET['ans']."' ");
...
$fetch1 = mysql_fetch_assoc($sql2);
		...
		$rows = mysql_fetch_assoc($sql2);
			...
			echo '<div class="entry" id="'.$fetchs['id'].'">
						...
						<div class="cnt">
							<h3 style="padding-bottom:0; line-height:30px;">Q. '.$question.'</h3>
							<p>Category : <b>'.$fetch1['name'].'</b><br></p>
...
?>
                    ...
                    <h4>Ans. </h4>
                       ...
                       <table align="center" width="96%" style="color:#333; font-family:Roboto,trebuchet ms,serif !important; font-size:16px;">
            	 
                                   
                                  
                                      <tr>
                                    			
                                    			<td><textarea class="resixeBox" onfocus="emptyElements('statu')"  type="text" id="question" placeholder="Write long description." style="min-height: 50px; margin-top:px;  border:1px solid #ff9031;  width:98%; padding:5px; font-family:Roboto,trebuchet ms,serif !important; font-size:16px;" ></textarea></td>
                                    </tr>
			  ...
			  function	submitQuest(){
				  var cat = <? echo $_GET['ans']?>;
				  var quest = document.getElementById('question').value;
				 
				  if(quest != ""){
						 $.post("sub_ans.php", {cat: cat, quest: quest}, function(data){
			...
			$sql = mysql_query("SELECT * FROM `answer` WHERE `ques_id`='".$_GET['ans']."' ORDER BY `id` DESC ");
		...
		$rows = mysql_fetch_assoc($sql2);
			...
			echo '<div class="entrys" id="'.$id.'">
						...
						<div class="cnt">';
						if($_SESSION['id'] == $uid || $_SESSION['id'] == '5'){
							echo '<img src="css/images/close.png" style="cursor:pointer; margin-top:10px; float:right;" onClick="deleteCat('.$id.')" height="10">';
						}
					echo '	<h4 style="font-size:16px; font-family:Roboto,trebuchet ms,serif !important; padding-top:10px;">'.$rows['name'].' '.$rows['lastnm'].'</h4><p>Answered on '.$datetime.'</p>
...
?>
                       ...
                       <script type="text/javascript">
							function deleteCat(x){
								...
								$.post("delete_ans.php",{x:x}, function(data){
									$('#'+x).hide('fast');
								});
