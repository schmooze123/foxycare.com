...
$DestinationDirectory	= 'bgimg/'; //Upload Directory ends with / (slash)
$Quality 				= 80;
...
if (is_array($ImageName)) 
{
	...
	for ($i=0; $i < $c; $i++)
	{
		...
		else
		{
			...
			$NewImageName = 'foxycare_image'.'_'.$RandomNumber.'_'.$RandomNumber2.'.'.$ImageExt;
			...
			if($processImage && resizeImage($CurWidth,$CurHeight,$BigImageMaxSize,$DestRandImageName,$CreatedImage,$Quality,$ImageType[$i]))
			{
				...
				if(!cropImage($CurWidth,$CurHeight,$ThumbSquareSize,$thumb_DestRandImageName,$CreatedImage,$Quality,$ImageType[$i]))
					...
					*/
					...
					$sql = mysql_query("INSERT INTO `foxycare`.`background` (`id`, `thumb`, `img`, `date`) VALUES (NULL, '".$thumb_DestRandImageName."', '".$DestRandImageName."', now())");
					
				
					
			echo'
		   
       <center> <img src="'.$DestRandImageName.'" style="max-width:100%; border:none; margin-bottom:5px; margin-top:5px; " /></center>
