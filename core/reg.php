						...
						if((strlen($email) >=10) && (strstr($email, "@")) && (strstr($email, "."))){
							...
							if($num_rows == 0){
								...
								if(strlen($pass) >= 5){
											...
											$activations = rand(time() ,1000);
											$activation = mysql_real_escape_string(htmlentities($activations)); 
											$insert_query = mysql_query("INSERT INTO `schmooze`.`users` (`id`, `username`, `fstnm`, `lstnm`, `friends_array`, `coursebk_array`, `fav_bk_array`, `talk_array`, `following`, `followed`, `coin`, `mood`, `email`, `pass`, `md5pass`, `favpost_array`, `status`, `activation`, `activate`, `profilepic`, `coverpic`, `login_ip`, `last_login`, `ip`, `date`) VALUES (NULL, '', '$firstnm', '$lastnm', '1', '', '', '', '1', '', '100', '23', '$email', '$pass', '$md5pass', '', '', '$activation', '0', 'images/Man-Brown-icon.png', '', '', '', '$ip', '$date')");
...
<div style="padding:24px; font-size:20px;">Hello '.$firstnm.' '.$lastnm.',<br /><br />Activation Code: '.$activation.' <br /><br />  Click the link Below to activate your account when ready:<br /><br /><a href="http://schmooze.co.in/activate.php?uid='.$uid.'">Click Here to activate your account now</a><br /><br />Login after successful activation using your:<br /> <br />E-mail Address: '.$email.'<br /></div> 
								...
								mail($to, $subject, $message, $headers);
									...
									echo $uid;
							//header('Location: select_color?u='.$username.'');
								}else
