...
<a href="latest_questions.php" style="padding:5px;"><li>Latest Question</li></a>
  ...
  function recover(){
	...
	window.location = "forgotten_password.php?email="+recover;  
           ...
           <div id="rightbar">
           <?
		   	if(empty($_GET['email']) & empty($_GET['user']) & empty($_GET['recover'])){
		   ?>
       <h3>Enter Your Email Id</h3>
               
    <table cellpadding="5" cellspacing="5" style="width:90%;   >
    
    	<tr style="">
        <td><input  id="recover_email" onfocus="emptyElement('status')"  placeholder="Your Email-Id" type="email"  style="height:35px; border-radius:2px; border:1px solid #ccc;  font-family:Roboto,trebuchet ms,serif !important; font-size:16px; width:80%; background:#FFF;"/></td>
            ...
            <td align="center">
              ...
              <input id="loginbtn" align="absmiddle" onclick="recover()" style="height:45px; width:150px; border-radius:3px; border:1px solid #ccc; float:right; background:#FFF; font-family:Roboto,trebuchet ms,serif !important; font-size:14px; background-image:url(css/images/red-btn.png); background-size:100% 105%; border:0; background-color:transparent; color:#FFF; margin-right:20%;  "   type="button"  value="Search" /></td>
        </tr>
         </table>
        ...
        <?
				...
				include 'core/connectdb.php';
				$sql10 = mysql_query("SELECT * FROM `users` WHERE `email`='".$_GET['email']."' LIMIT 1" );
				$rowsz = mysql_fetch_assoc($sql10);
				$getid = $rowsz['id'];
				$getfirstname = $rowsz['name'];
				$getlastname = $rowsz['lastnm'];
				$getprofilepic = $rowsz['profilepic'];
				
				
		?>
			 <div style="height:400px; width:500px; border:px solid #000;">
                 <p align="left" style="font-family:Roboto,trebuchet ms,serif !important; margin:5px; padding:5px; font-size:20px; color:#333; margin-top:2px;">Reset Your Password</p>
               <hr style="margin-bottom:10px;" />
           	  
               <h2 align="center" style="font-family:Roboto,trebuchet ms,serif !important; color:#000; margin-top:15px;"><? echo $getfirstname, ' ', $getlastname?></h2>
                      <p align="center" style="font-family:Roboto,trebuchet ms,serif !important; font-size:14px;  margin-top:2px;">Email me a link to reset my password</p>
                       <p align="center" style="font-family:Roboto,trebuchet ms,serif !important; font-size:14px;  margin-top:2px;"><? echo $_GET['email']?></p>
               <input id="loginbtn" align="absmiddle" onclick="sendCode(<? echo $getid?>)" style="height:45px; width:150px;  float:right; font-family:Roboto,trebuchet ms,serif !important; font-size:14px; background-image:url(css/images/red-btn.png); background-size:100% 105%; border:0; background-color:transparent; color:#FFF;; margin-right:10px; margin-top:10px; "   type="button"  value="Continue" />
             </div>
            <?
    	...
    	<tr style="">
        <td><input  id="recoverCode" onfocus="emptyElement('status')"  placeholder="Your Code" type="text"  style="height:35px; border-radius:2px; border:1px solid #ccc;  font-family:Roboto,trebuchet ms,serif !important; font-size:15px; width:80%; background:#FFF;"/></td>
            ...
            <td align="center">
              ...
              <input id="loginbtn" align="absmiddle" onclick="submitcode(<? echo $_GET['user']?>)" style="height:45px; width:150px;  float:right; font-family:Roboto,trebuchet ms,serif !important; font-size:14px; background-image:url(css/images/red-btn.png); background-size:100% 105%; border:0; background-color:transparent; color:#FFF; margin-right:20%;  "   type="button"  value="Submit" /></td>
				...
				?>
                  <table cellpadding="5" cellspacing="5" style="margin-top:20px; width:90%; margin-top:150px; float:right;" align="center">
    
    	<tr style="">
        <td><input  id="recoverCode" onfocus="emptyElement('status')"  placeholder="Enter password" type="text"  style="height:35px; border-radius:2px; border:1px solid #FFF; box-shadow:rgba(255,255,255,1) 0 0 5px; font-family:Roboto,trebuchet ms,serif !important; font-size:15px; width:80%; background:#FFF;"/></td>
          
    	</tr>
       <tr style="">
        <td><input  id="recoverCode" onfocus="emptyElement('status')"  placeholder="Re-enter password" type="text"  style="height:35px; border-radius:2px; border:1px solid #FFF; box-shadow:rgba(255,255,255,1) 0 0 5px; font-family:Roboto,trebuchet ms,serif !important; font-size:15px; width:80%; background:#FFF;"/></td>
            ...
            <td align="center">
              <div id="status" style="margin-bottom:5px; color:#F00; font-family:rosemary; font-size:14px; "></div>
              <input id="loginbtn" align="absmiddle" onclick="submitcode(<? echo $_GET['user']?>)" style="height:35px; width:150px; border-radius:3px; border:1px solid #FFF; float:right; background:#FFF; font-family:Roboto,trebuchet ms,serif !important; margin-right:20%; font-size:15px; color:#333; box-shadow:rgba(255,255,255,1) 0 0 3px;"   type="button"  value="Submit" /></td>
        </tr>
         </table>
                <?
      ...
      <script>
		   function sendCode(id){
			 $.post("recover_code.php", {id: id}, function(data){
				 window.location = "forgotten_password.php?user="+data; 
			 });
		   }
		   </script>
            <script>
		   function submitcode(id){
			 ...
			 $.post("check_code.php", {id: id, recoverCode: recoverCode}, function(data){
				// window.location = "forgotten_password.php?recover="+data; 
				if(data == 1){
					window.location = "recover_pass.php?user="+<? echo $_GET['user']?>+"&recover="+recoverCode;
				}else{
					return false;
				}
			 });
